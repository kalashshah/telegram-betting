...
contract MarketAirDao {
    ...
    event PullRequest(bytes32 id, string description);
    ...
    function endOption(bytes32 id, uint _outcome) public onlyOwner {
        emit PullRequest(id, options[id].description);
    }

    function fulfillRequest(bytes32 id, uint _outcome) public onlyOwner {
        Option storage option = options[id];
